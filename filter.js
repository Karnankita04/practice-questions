...
const isLengthMoreThan5 = function (word) {
  return isGreater(word.length)(5);
};

const filterLongWords = function (words) {
  return words.filter(isLengthMoreThan5);
};

// people older than 30 [{name: "Alice", age: 25}, {name: "Bob", age: 35}] => [{name: "Bob", age: 35}]

const isAbove30 = function (object) {
  return isGreater(object.age)(30);
};
...
const arePagesMoreThan200 = function (bookDetail) {
  return isGreater(bookDetail.pages)(200);
};
...
const isGradeAbove80 = function (student) {
  return isGreater(student.grade)(80);
};
...
const arePlacedInLast30Days = function (order) {
  ...
  const are30DaysFromLastToCurrMonth =
    currMonth - givenMonth === 1 && days - givenDay + currDay <= 30;

  const areLessThanOrEqualTo30DaysInCurrMonth =
    currMonth - givenMonth === 0 && currDay - givenDay < 30;

  return (
    (givenYear === currYear &&
      currMonth - givenMonth <= 1 &&
      are30DaysFromLastToCurrMonth) ||
    areLessThanOrEqualTo30DaysInCurrMonth
  );
};
...
const getMedian = function (values) {
  ...
  const middleElements = isEven(values.length)
